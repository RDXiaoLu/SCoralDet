...
class yolov8_heatmap:
    ...
    def draw_detections(self, box, color, name, img):
        ...
        # 查找第一个空格或左括号之前的部分作为类别名
        if ' ' in name:
            class_name = name.split(' ')[0].strip()
        else:
            class_name = name.strip()
        cv2.putText(img, class_name, (xmin, ymin - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, tuple(int(x) for x in color), 2, lineType=cv2.LINE_AA)
...
def get_params():
    params = {
        'weight': 'YOLOv10/weights/best.pt', # 现在只需要指定权重即可,不需要指定cfg
        'device': 'cuda:0',
...
if __name__ == '__main__':
    ...
    # model(r'9999963_00000_d_0000020.jpg', 'result')
    model(r'/mnt/hdd/yuanhui/data/coral_soft/images/train/Sarcophyton_56.JPG', 'result')
